...
class Orders extends Component {
    ...
    async apicall() 
    {
        fetch('http://menu.pickmycaterer.com/apivendor/orderapi',{
        method: 'POST',
        header:{
            'Accept': 'application/json',
            'Content-Type': 'application/json'
        },
        body: JSON.stringify({"vendor_email":this.state.value})
        })
        .then((response) => response.json())
        .then((res) => {
            ...
            if(res.is_error == 1)
            {
                this.setState({loading:false ,error:'1',orders:[]})
            }
            else
            {
                console.log(res)
    ...
    searchFilterFunction = text => {
        ...
        if(text)
        {
            ...
            const itemData = item.order_token.toLowerCase();
        ...
        return(
            <View style={styles.container}>
                ...
                <View refreshControl={<RefreshControl refreshing={this.state.refreshing} onRefresh={onRefresh} />} style={{backgroundColor:'#FFF' , flex:1 , padding:30}} >
                    ...
                    <Image style={styles.images} source={require('../../assets/images/box.png')} />
                        ...
                        <View key={ index }>        
                            <View style={styles.listview}>
                                <View style={styles.orders}>
                                    <Text style={styles.ordertext}>{item.order_token}</Text>
                                    <Text>New</Text>
                                </View>
                                <View style={styles.actions}>
                                    <Button icon="eye"  onPress={() => this.props.navigation.navigate('Screen1',{order_token: item.order_token})}>View</Button>
                                </View>
                            </View>
                            <Divider />
                        </View>
